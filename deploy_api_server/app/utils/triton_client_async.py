...
class BaseTritonClient(abc.ABC):
    ...
    def _generate_completion_outputs(
        ...
        """Generate the expected text output structure."""
        return [self._infer_output("text_output")]

    def _generate_embedding_outputs(
        self,
    ) -> List[Union[grpcclient.InferRequestedOutput, httpclient.InferRequestedOutput]]:
        """Generate the expected embedding output structure."""
        return [self._infer_output("embedding")]
    ...
    def _generate_completion_inputs(  # pylint: disable=too-many-arguments,too-many-locals
        self,
        prompt: str,
        stop_words: List[str] = ["</s>", "<|end_of_text|>", "<|eot_id|>"],
        max_tokens: int = 512,
        ...
        input_random_seed = np.array([seed]).astype(np.uint64).reshape((1, -1))
    ...
    def _generate_embedding_inputs(  # pylint: disable=too-many-arguments,too-many-locals
        ...
        if isinstance(input, str):
            input = [input]
        input_tensor = np.array(input).astype(object).reshape((1, -1))

        inputs = [self._prepare_tensor("input", input_tensor)]
        return inputs
...
class GrpcTritonClient(BaseTritonClient):
    ...
    @staticmethod
    def _process_completion_result(result: Dict[str, str]) -> str:
    ...
    def _stream_callback(
        ...
        if error:
            ...
            if "outputs" in response_raw:
                ...
                response = self._process_completion_result(response_raw)
                if force_batch:
